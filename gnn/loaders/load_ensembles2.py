...
class EnsembleGraph(InMemoryDataset):
    ...
    def processed_file_names(self):
        ...
        return [
            f"data-ensemble2-{filename}-{self.split}-{self.add_full}-{self.full_split}-{self.train_size}-{self.hot}-{self.algorithm}-{self.num_neighbours}-{self.scaled}-{self.remove_mean}-{self.smoothing}-{self.mode}-{self.use_weights}-{self.use_validation}-{self.include_self}-{self.separate_sets}-{bits}.pt"
        ]
        ...
        for filename in self.raw_file_names:
            ...
            if self.use_validation:
                valid_size = (
                    ...
                    else int(df_whole.shape[0] * self.validation_size)
                )
                df_whole = df_whole[valid_size:]
                df_valid = df_whole[:valid_size]

            full_split = (
                int(self.full_split * len(df_whole))
                if isinstance(self.full_split, float)
                else self.full_split
            )
            ...
            train_size = (
                ...
                else self.train_size / df_whole.shape[0]
            )

            if self.separate_sets:
                whole_train = df_whole.sample(frac=train_size).reset_index(drop=True)
                whole_test = df_whole.loc[
                    ~df_whole.index.isin(whole_train.index)
            ...
            for split in range(self.split + (1 if self.add_full else 0)):
                if split == self.split and self.add_full:
                    if full_split is None:
                        df_train = df_whole.sample(
                            frac=train_size, random_state=split
                        ).reset_index(drop=True)
                    ...
                    if self.separate_sets:
                        df_train = whole_train.sample(
                            frac=0.5, random_state=split
                        ).reset_index(drop=True)
                        df_test = whole_test
                    else:
                        if full_split is None:
                            ...
                            df_test = df_whole.loc[
                                ~df_whole.index.isin(df_train.index)
                            ].reset_index(drop=True)
                        else:
                            ...
                            train_size = (
                                self.train_size
                                if self.train_size < 1
                                else self.train_size / df_whole_less_full.shape[0]
                            )
                            df_train = df_whole_less_full.sample(
                                frac=train_size, random_state=split
                            ).reset_index(drop=True)
                            df_test = df_whole_less_full.loc[
                                ~df_whole_less_full.index.isin(df_train.index)
                            ].reset_index(drop=True)
                ...
                if self.hot:
                    ...
                    for i in range(1, len(df_xtrain.columns)):
                        df_xtrain[i + len(df_xtrain.columns) - 1] = (
                            df_xtrain[conv[i - 1]] == 2
                        ).astype(int)
                    for i in range(1, len(df_xtest.columns)):
                        df_xtest[i] = (df_xtest[conv[i - 1]] == 1).astype(int)
                    for i in range(1, len(df_xtest.columns)):
                        df_xtest[i + len(df_xtest.columns) - 1] = (
                            df_xtest[conv[i - 1]] == 2
                        ).astype(int)
                ...
                y_train = torch.tensor(
                    [[n] for n in df_ytrain.values], dtype=torch.float
                )
                ...
                knn_dist_graph_test = kneighbors_graph(
                    X=df_xtest,
                    n_neighbors=n_neighbors,
                    mode=self.mode,
                    metric=self.algorithm,
                ...
                if self.smoothing == "laplacian":
                    ...
                    normalized = np.asarray(
                        knn_dist_graph_train[nonzeroindices]
                        / np.max(knn_dist_graph_train[nonzeroindices])
                    )
                    ...
                    if self.use_weights:
                        data.edge_weight = (
                            1 - abs(edge_weight) / max(edge_weight).float()
                        )
                    ...
                    normalized = np.asarray(
                        knn_dist_graph_test[nonzeroindices]
                        / np.max(knn_dist_graph_test[nonzeroindices])
                    )
                    similarity_graph[nonzeroindices] = np.exp(
                        -np.asarray(normalized) ** 2 / 2.0 * sigma**2
                    )
            ...
            if self.use_validation:
                ...
                y_valid = torch.tensor(
                    [[n] for n in df_yvalid.values], dtype=torch.float
                )

                knn_dist_graph_valid = kneighbors_graph(
                    X=df_xvalid,
                    n_neighbors=n_neighbors,
                    mode=self.mode,
                    metric=self.algorithm,
                ...
                if self.smoothing == "laplacian":
                    ...
                    normalized = np.asarray(
                        knn_dist_graph_valid[nonzeroindices]
                        / np.max(knn_dist_graph_valid[nonzeroindices])
                    )
