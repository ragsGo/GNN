...
def create_sibling_edges(filename, subset):
    ...
    for x in range(df.shape[0]):
        for y in range(x, df.shape[0]//2):
            edges[x, y] += 0.25 if df.iloc[x]["mum"] == df.iloc[y]["mum"] else 0
            edges[x, y] += 0.25 if df.iloc[x]["father"] == df.iloc[y]["father"] else 0
...
class MyOwnDataset(InMemoryDataset):
    ...
    def processed_file_names(self):
        ...
        return [f'data-{filename}-{self.use_validation}-{self.num_neighbours}-{self.smoothing}-{self.mode}-{bits}.pt']
    ...
    def process(self):
        ...
        for filename in self.raw_file_names:
            ...
            sibling_train = create_sibling_edges("Pedigree_Data.csv", slice(0, train_set))
            print("DONE WITH TRAIN")
            sibling_test = create_sibling_edges("Pedigree_Data.csv", slice(train_set, test_set))

            if self.smoothing == "laplacian":
                sigma = 1
                similarity_graph = sparse.csr_matrix(sibling_train.shape)
                nonzeroindices = sibling_train.nonzero()
                similarity_graph[nonzeroindices] = np.exp(
                    -np.asarray(sibling_train[nonzeroindices]) ** 2 / 2.0 * sigma ** 2)
                ...
                sigma = 1
                similarity_graph = sparse.csr_matrix(sibling_test.shape)
                nonzeroindices = sibling_test.nonzero()
                similarity_graph[nonzeroindices] = np.exp(
                    -np.asarray(sibling_test[nonzeroindices]) ** 2 / 2.0 * sigma ** 2)
                similarity_graph = 0.5 * (similarity_graph + similarity_graph.T)
                graph_laplacian_s = sparse.csgraph.laplacian(csgraph=similarity_graph, normed=False)
                graph_laplacian = graph_laplacian_s.toarray()
                ...
                edge_index = torch.tensor(sibling_train.nonzero(), dtype=torch.long)
                data = Data(x=x_train, y=y_train, edge_index=edge_index)

                edge_index = torch.tensor(sibling_test.nonzero(), dtype=torch.long)
...
def load_data_siblings(filename, bits=None, num_neighbours=1, smoothing="laplacian", mode="connectivity", use_validation=False):
