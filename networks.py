...
def create_network_no_conv_relu(inp_size, out_size, conv_kernel_size=30, pool_size=2, internal_size=-1):
    if internal_size <= 0:
...
# def create_network_two_no_conv_dropout(inp_size, out_size, conv_kernel_size=30, pool_size=2, internal_size=-1,
#                                         dropout=0.5):
#     if internal_size <= 0:
#         internal_size = inp_size
#     out_pool = internal_size
...
def create_network_two_no_conv_relu_dropout(inp_size, out_size, internal_size=-1,
                                        dropout=0.5, add_self_loops=True, **_):
    if internal_size <= 0:
        internal_size = inp_size
    out_pool = internal_size
    model = Sequential('x, edge_index, edge_weights?', [
...
def create_network_two_dense_no_conv_dropout(inp_size, out_size, internal_size=-1,
                                        dropout=0.5, **_):
    if internal_size <= 0:
        internal_size = inp_size
    out_pool = internal_size
...
def create_network_graph_conv_two_dropout(inp_size, out_size, conv_kernel_size=30, pool_size=2, internal_size=-1, dropout=0.5):
    if internal_size <= 0:
        internal_size = inp_size
...
def create_network_conv_cov(inp_size, out_size, conv_kernel_size=30, pool_size=2, internal_size=-1, max_neighbours=2,
                            radius=2):
    if internal_size <= 0:
        internal_size = inp_size
    out_conv = internal_size
...
def create_network_no_conv_cov(inp_size, out_size, conv_kernel_size=30, pool_size=2, internal_size=-1, max_neighbours=2,
                               radius=2):
    if internal_size <= 0:
        internal_size = inp_size
    out_pool = internal_size
...
def create_network_2D_graph(inp_size,out_size, embedding_dim, filter_size, conv_kernel_size=37, pool_size=2, internal_size=-1, **_):
    if internal_size <= 0:
        internal_size = inp_size
    ...
    model = Sequential(
        ...
        [(Reshape(1, inp_size), 'x -> x'),Embed(inp_size,embedding_dim),Squeeze(1),Print(),
        (Conv2d(in_channels=1, out_channels=conv_kernel_size,stride=1, padding=1, kernel_size=(filter_size[0], embedding_dim)), 'x -> x'),
        (Conv2d(in_channels=1, out_channels=conv_kernel_size, stride=1, padding=1, kernel_size=(filter_size[1], embedding_dim)), 'x -> x'),
        (Conv2d(in_channels=1, out_channels=conv_kernel_size, stride=1, padding=1, kernel_size=(filter_size[2], embedding_dim)), 'x -> x'),
        Print(),
        ReLU(),
        Print(),
        MaxPool1d(kernel_size=pool_size),
...
def create_network_no_graph(inp_size, out_size, conv_kernel_size=37, pool_size=2, internal_size=-1, **_):
    ...
    out_pool = int((out_conv + 2 * 0 - 1 * (pool_size - 1) - 1) / pool_size + 1)
    print('outpool===', out_pool)
    # print(((inp_size - conv_kernel_size)/(conv_kernel_size + 1)))
    #prints
    model = Sequential('x, edge_index, edge_weights?', [
        (Reshape(1, inp_size), 'x -> x'),
...
def create_network_ones_hop(inp_size, out_size, conv_kernel_size=37, pool_size=2, internal_size=-1, radius=2, as_double=False):
    if internal_size <= 0:
        internal_size = inp_size
    internal_size += internal_size % 2
...
def create_network_ones(inp_size, out_size, conv_kernel_size=37, pool_size=2, internal_size=-1, as_double=False):
    if internal_size <= 0:
        internal_size = inp_size
    internal_size += internal_size % 2
...
def create_network_conv_1D(inp_size, out_size, conv_kernel_size=4, pool_size=2, internal_size=-1, filters=1, as_double=False):
    #print('out---',out_size)
    ...
    model = nn.Sequential(
        Reshape(1, 1, inp_size),Print(),
        Conv1d(in_channels=1, out_channels=filters, kernel_size=(1, conv_kernel_size),  padding=0),Print(),
        ReLU(inplace=True),
...
def create_network_conv2D(inp_size, out_size, conv_kernel_size=4, pool_size=2, internal_size=-1, filters=1, as_double=False):
    #print('out---',out_size)
