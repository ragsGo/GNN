...
class MyOwnDataset(InMemoryDataset):
    def __init__(self, root, bits, raw_filenames, num_neighbours=1, smoothing="laplacian", mode="connectivity", split=None, scaled=False, remove_mean=True, hot=False):
        self.num_neighbours = num_neighbours
        self.smoothing = smoothing
        self.mode = mode
        self.raw_filenames = raw_filenames if not isinstance(raw_filenames, str) else [raw_filenames]
        self.bits = bits
        self.hot = hot
        self.scaled = scaled
    ...
    def processed_file_names(self):
        ...
        filenames = str(len(self.raw_filenames))
        split = ".".join([os.path.basename(x) for x in self.split["test"]]) if isinstance(self.split, dict) else self.split
        return [f'data-many-{filenames}-{split}-{self.num_neighbours}-{self.smoothing}-{self.mode}-{self.scaled}-{self.remove_mean}-{self.hot}-{bits}.pt']
    ...
    def process(self):
        ...
        print({
            ...
            "mode": self.mode,
            "raw_filename": self.raw_filenames,
            "bits": self.bits,
            "hot": self.hot,
            "scaled": self.scaled,
        ...
        for filename in self.raw_file_names:
            ...
            if isinstance(self.split, (int, float)):
                ...
                train_set = split
                test_set = df_whole.shape[0]
            else:
                if filename in self.split["test"]:
                    train_set = 0
                    test_set = df_whole.shape[0]
                else:
                    train_set = df_whole.shape[0]
                    test_set = df_whole.shape[0]

            if df_xtrain is None:
                df_xtrain = df_whole.iloc[0:train_set, 1:]
                df_ytrain = df_whole['value'][0:train_set]
            else:
                df_xtrain = pd.concat([df_xtrain, df_whole.iloc[0:train_set, 1:]])
                df_ytrain = pd.concat([df_ytrain, df_whole['value'][0:train_set]])

            if df_xtest is None:
                df_xtest = df_whole.iloc[train_set:test_set, 1:]
                df_ytest = df_whole['value'][train_set:test_set]
            else:
                df_xtest = pd.concat([df_xtest, df_whole.iloc[train_set:test_set, 1:]])
                df_ytest = pd.concat([df_ytest, df_whole['value'][train_set:test_set]])
        ...
        try:
            df_final = df_xtrain.copy()
            df_final["value"] = df_ytrain.values
            df_temp = df_xtest.copy()
            df_temp["value"] = df_ytest.values
            df_final = pd.concat([df_final, df_temp])
            df_final.to_csv("dataframes/" + self.processed_file_names[0] + ".csv", columns=labels, header=False, index=False)
        except:
            raise
...
def load_data_many(filename, bits=None, num_neighbours=1, smoothing="laplacian", mode="connectivity", split=None, scaled=False, remove_mean=True, hot=False, **_):
    return MyOwnDataset(".", bits=bits, raw_filenames=filename, num_neighbours=num_neighbours, smoothing=smoothing, mode=mode, split=split, scaled=scaled, remove_mean=remove_mean, hot=hot)  # loader.dataset


load_data = load_data_many
