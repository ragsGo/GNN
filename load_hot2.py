...
class MyOwnDataset(InMemoryDataset):
    ...
    def processed_file_names(self):
        ...
        return [f'hot2-data-{filename}-{self.split}-{self.use_validation}-{self.num_neighbours}-{self.smoothing}-{self.mode}-{bits}.pt']
    ...
    def process(self):
        ...
        for filename in self.raw_file_names:
            ...
            for i in range(1, len(df_whole.columns)):
                df_new[i] = (df_whole[conv[i-1]] == 1).astype(int)
                labels.append(i)
            for i in range(1, len(df_whole.columns)):
                df_new[i+len(df_whole.columns)-1] = (df_whole[conv[i-1]] == 2).astype(int)
...
def load_data_hot2(filename, bits=None, num_neighbours=1, smoothing="laplacian", mode="connectivity", use_validation=False, split=None):
