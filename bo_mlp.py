...
from networks import create_network_linear,create_network_no_graph
...
def train_evaluate(parameters, device ="cpu"):
    data, num_features = create_data(load_data)
    poolsz = parameters.get("plsz", 2)
    kr_sz = parameters.get("krsz", 30)
    model = create_network_no_graph(num_features, 1, conv_kernel_size=kr_sz, pool_size=poolsz)
    print('lalal,',model)
    ...
    for epoch in range(250):
        ...
        l1_lambda = parameters.get("l1", 0.3)
