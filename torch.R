...
# test_dl = testF %>% dataloader(batch_size = 400, shuffle = FALSE)
print(object.size(train_dl))
print(object.size(valid_dl))
# print(object.size(test_dl))
#Added new layer to change the dimension and rotate so the batch is in the right dimension
unsqueeze <- nn_module(
  initialize = function(dim) {
    self$dim <- dim
  },
  forward = function(x) {
     x <- torch_rot90(x, 1, c(1,2))$unsqueeze(self$dim)
     x
  }
)
#The 1d Conv Net
model <- nn_module(
  initialize = function() {
    self$features <- nn_sequential(
      unsqueeze(2), #new function
      nn_conv1d(1, 1, kernel_size = 3, padding = 1), # the first parameter is the input channel and the second paraneter is output. here its bnot image and has one channel.
      nn_relu(),
      nn_adaptive_max_pool1d(2),
      nn_conv1d(1, 1, kernel_size = 3, padding = 1), #number of filter
      nn_relu(),
      nn_adaptive_max_pool1d(2),
      nn_dropout(0.3),
      nn_flatten(1)
    ...
    self$classifier <- nn_sequential(
      nn_linear(5012, 1) # this has to be calculated based on pooling and kernel size, padding.
    )
  },
  forward = function(x) {
    x <- self$features(x)
